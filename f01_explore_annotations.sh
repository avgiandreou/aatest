...
explore_annotations(){
	...
	bcftools query -H -f '%ID\t%vep_SYMBOL\t%vep_Consequence\t%vep_IMPACT\t%CLNSIG\t%CLNREVSTAT\t%CLNDN\t%vep_SIFT\t%vep_PolyPhen\t%vep_CADD_PHRED\t%vep_gnomAD_NFE_AF\t%vep_gnomAD_AF\t%HWE\t%ALLELEID\t%vep_Existing_variation\t%QUAL\t%FILTER\t%AF\n' "${vcf}" &> "${annotations}"

	echo "--- FILTER ---"
	echo ""
	awk 'NR>1 {print $17}' "${annotations}" | sort |  uniq -c | sort -nr
	echo ""
	...
	echo ""
	echo "Missed NFE AFs:"
	awk 'NR>1 {print $12}' "${annotations}" | grep -xc "\."
	echo ""
	echo "Zero NFE AFs:"
	awk 'NR>1 {print $12}' "${annotations}" | grep -xc 0
	echo ""
	echo "Non-missed non-zero NFE AFs:"
	awk 'NR>1 {print $12}' "${annotations}" | grep -vxc [0\.]
	echo ""
	awk 'NR>1 {print $12}' "${annotations}" | grep -vx [0\.]  | sort -g | head
	echo "..."
	awk 'NR>1 {print $12}' "${annotations}" | grep -vx [0\.] | sort -g | tail
	echo ""
